...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.ObraDeArte.objects.filter(ceramica=True).update(disciplina='CERAMICA')
        orm.ObraDeArte.objects.filter(dibujo=True).update(disciplina='DIBUJO')
        orm.ObraDeArte.objects.filter(escultura=True).update(disciplina='ESCULTURA')
        orm.ObraDeArte.objects.filter(fotografia=True).update(disciplina='FOTOGRAFIA')
        orm.ObraDeArte.objects.filter(grabado=True).update(disciplina='GRABADO')
        orm.ObraDeArte.objects.filter(litografia=True).update(disciplina='LITOGRAFIA')
        orm.DisciplinaArtistica.objects.create(disciplina='OTROS')
        orm.ObraDeArte.objects.filter(otros=True).update(disciplina='OTROS')
