...
def _verify_cas2_saml(ticket, service):
    ...
    try:
        ...
        if success is not None and success.attrib['Value'].endswith(':Success'):
            ...
            for at in attrs:
                if 'username' in at.attrib.values():
...
class CASBackend(ModelBackend):
    ...
    def authenticate(self, ticket, service, request):
        ...
        try:
            ...
            if 'first_name' in request.session['attributes']:
                user.first_name = request.session['attributes']['first_name']
            if 'last_name' in request.session['attributes']:
                user.last_name = request.session['attributes']['last_name']
            if 'email' in request.session['attributes']:
